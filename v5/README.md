...
A Retrieval-Augmented Generation (RAG) system that answers questions about the Bitcoin and Ethereum whitepapers using LangChain, Pinecone, and OpenAI. v2 adds a Gradio chat interface. v3 adds RAGAS evaluation. v4 adds multi-query + reranking with a before/after comparison. v5 adds adaptive retrieval, which runs the cheap baseline first and only escalates to multi-query + reranking when confidence is low.
...
- **Three-way RAGAS comparison** — Baseline vs Multi-query vs Adaptive with delta columns
